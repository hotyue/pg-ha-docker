...
- XPOST: 告诉 Patroni 我们要发起一个动作。

- leader: 当前的领袖（node2）。

- candidate: 你想要提拔的新领袖（比如 node3）。
...
- 动作逻辑：
   ...
   - Patroni 接收到请求后，会先停止该节点上的 PostgreSQL。

   - 它会彻底清空该节点的数据目录 (data_dir)。

   - 随后自动触发 pg_basebackup 或你配置的备份恢复工具，从当前的 Leader 重新拉取全量数据。

- 适用场景：

   - 从库报 Timeline mismatch（时间线不匹配）。

   - 从库数据物理损坏，无法通过 WAL 正常追赶。

   - 你手动在从库乱动了数据，导致主从校验失败。
